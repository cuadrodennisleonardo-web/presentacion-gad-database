...
type JusticeStat = Database['public']['Tables']['justice_stats']['Row'];
type TabType = string;
...
export default function JusticeDataEntry() {
  ...
  const [stats, setStats] = useState<Record<string, Partial<JusticeStat>>>({});
  const [originalStats, setOriginalStats] = useState<Record<string, Partial<JusticeStat>>>({});
  const [activeTab, setActiveTab] = useState<TabType>('main');
  const [year, setYear] = useState(getDefaultYear(`Justice & Safety_${'main'}`));

  useEffect(() => {
    setYear(getDefaultYear('Justice & Safety_' + activeTab));
  }, [activeTab]);
  
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [pendingChanges, setPendingChanges] = useState<Record<string, any> | null>(null);

  const nativeTabs = [{ key: 'main', label: 'Peace & Justice' }];
  ...
  const tabLabel = activeTab === 'main' ? 'Peace & Justice' : (dynamicSchemas.find(d => d.id === activeTab)?.tab_name || '');
  ...
  const { data: fetchedData, isLoading } = useDataEntryStats('Justice & Safety', 'justice_stats', year);
  ...
  const mutation = useMutation({
    mutationFn: async (changedData: Record<string, any>) => {
      if (isSuperAdmin) {
        ...
        const { error } = await supabase.from('justice_stats').upsert(upsertData, { onConflict: 'barangay_id,year' });
        if (error) throw error;
        
        await notifySuperAdminsOfDirectSave('Justice & Safety', 'Peace & Justice', year, user!.id);
      } else {
        await submitForApproval('Justice & Safety', 'Peace & Justice', year, changedData, user!.id);
      }
    },
    onSuccess: () => {
      toast.success(isSuperAdmin ? `Justice & Safety data saved directly!` : `Changes submitted for approval!`);
      queryClient.invalidateQueries({ queryKey: ['native_data', 'Justice & Safety', year] });
      queryClient.invalidateQueries({ queryKey: ['latest_approval', 'Justice & Safety', tabLabel, year] });
      queryClient.invalidateQueries({ queryKey: ['justice_stats', year] });
  ...
  return (
    <DataEntryLayout
      ...
      pageTitle="Justice Data Entry"
      pageDescription="Manage Justice stats"
      breadcrumbTitle={`Justice & Safety ${!canWrite ? 'View Data' : 'Data Entry'}`}
      gridTitle="Peace & Justice Grid"
      gridDescription={`Manage peace and order data for ${barangays.length} barangays. Enter Sex-Disaggregated Data.`}
      year={year}
      setYear={setYear}
      activeTab={activeTab}
      ...
      exportTitle={`Justice & Safety (${year})`}
      ...
      <div className="hidden lg:block w-full max-w-full min-w-0 overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-800">
        ...
        <thead className="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-gray-800/50 dark:text-gray-400">
               <tr>
                 ...
                 <th className="whitespace-nowrap px-4 py-2 font-medium text-center border-l dark:border-gray-800 text-blue-600 dark:text-blue-400" colSpan={3}>Children in Conflict (CICL)</th>
                 <th className="whitespace-nowrap px-4 py-2 font-medium text-center border-l border-r dark:border-gray-800 text-indigo-600 dark:text-indigo-400" colSpan={3}>Sexual Assault Victims</th>
                 <th className="whitespace-nowrap px-4 py-2 font-medium text-center border-r dark:border-gray-800 text-rose-600 dark:text-rose-400" rowSpan={2}>VAWC Reported</th>
               </tr>
             <tr>
              ...
              <tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                ...
                <td className="px-0 py-0 bg-rose-50/30 dark:bg-rose-900/10">
                  <input 
                    ...
                    onChange={(e) => handleChange(b.id, 'vawc_cases_reported', e.target.value)} 
                    disabled={!canWrite || isLocked} 
                    className="w-full min-w-[60px] bg-transparent px-2 py-2 text-center outline-none focus:bg-brand-50 dark:focus:bg-brand-900/20 disabled:opacity-100 disabled:text-gray-900 dark:disabled:text-white" 
                  />
                </td>
        ...
        <tfoot className="bg-amber-50/80 dark:bg-amber-950/30 font-bold border-t-2 border-amber-300 dark:border-amber-800 text-gray-900 dark:text-white">
          {(() => {
            ...
            const c = calc('cicl');
            const sa = calc('sexual_assault');
            let vawcSum = 0;
            barangays.forEach(b => {
              const r = stats[b.id] || {};
              vawcSum += Number(r.vawc_cases_reported || 0);
            });

            return (
              <tr>
                <td className="whitespace-nowrap px-4 py-3 font-extrabold text-brand-700 dark:text-brand-300">Total</td>
                <td className="px-2 py-3 text-center border-l dark:border-gray-800 text-blue-700 dark:text-blue-300 font-bold">{c.mSum.toLocaleString()}</td>
                <td className="px-2 py-3 text-center text-blue-700 dark:text-blue-300 font-bold">{c.fSum.toLocaleString()}</td>
                <td className="px-4 py-3 text-center font-extrabold bg-amber-100/80 dark:bg-amber-900/50 text-amber-900 dark:text-amber-200">{c.totSum.toLocaleString()}</td>

                <td className="px-2 py-3 text-center border-l dark:border-gray-800 text-indigo-700 dark:text-indigo-300 font-bold">{sa.mSum.toLocaleString()}</td>
                <td className="px-2 py-3 text-center text-indigo-700 dark:text-indigo-300 font-bold">{sa.fSum.toLocaleString()}</td>
                <td className="px-4 py-3 text-center font-extrabold bg-amber-100/80 dark:bg-amber-900/50 text-amber-900 dark:text-amber-200 border-r dark:border-gray-800">{sa.totSum.toLocaleString()}</td>

                <td className="px-4 py-3 text-center font-extrabold text-rose-700 dark:text-rose-300">{vawcSum.toLocaleString()}</td>
              </tr>
            );
          })()}
        </tfoot>
        ...
        {barangays.map((b) => {
          ...
          return (
            <MobileDataCard key={b.id} title={b.name}>
              <MobileDataInput label="Children in Conflict (CICL)" type="split">
                <SplitInputWrapper
                  inputMale={<input type="number" min="0" value={row['cicl_m'] || ''} onChange={(e) => handleChange(b.id, 'cicl_m', e.target.value)} disabled={!canWrite || isLocked} className="w-full px-2 py-2 rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-sm text-center text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500" />}
                  inputFemale={<input type="number" min="0" value={row['cicl_f'] || ''} onChange={(e) => handleChange(b.id, 'cicl_f', e.target.value)} disabled={!canWrite || isLocked} className="w-full px-2 py-2 rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-sm text-center text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500" />}
                  total={(row.cicl_m || 0) + (row.cicl_f || 0)}
                />
              </MobileDataInput>
              <MobileDataInput label="Sexual Assault Victims" type="split">
                <SplitInputWrapper
                  inputMale={<input type="number" min="0" value={row['sexual_assault_m'] || ''} onChange={(e) => handleChange(b.id, 'sexual_assault_m', e.target.value)} disabled={!canWrite || isLocked} className="w-full px-2 py-2 rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-sm text-center text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500" />}
                  inputFemale={<input type="number" min="0" value={row['sexual_assault_f'] || ''} onChange={(e) => handleChange(b.id, 'sexual_assault_f', e.target.value)} disabled={!canWrite || isLocked} className="w-full px-2 py-2 rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-sm text-center text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500" />}
                  total={(row.sexual_assault_m || 0) + (row.sexual_assault_f || 0)}
                />
              </MobileDataInput>
              <MobileDataInput label="VAWC Reported">
                <input type="number" min="0" value={row['vawc_cases_reported'] || ''} onChange={(e) => handleChange(b.id, 'vawc_cases_reported', e.target.value)} disabled={!canWrite || isLocked} className="w-full px-2 py-2 rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 text-sm text-center text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500" />
              </MobileDataInput>
            </MobileDataCard>
          );
        })}
