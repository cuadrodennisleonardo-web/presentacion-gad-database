...
const CATEGORIES: { id: FeedbackCategory; label: string; icon: React.FC<{ className?: string }>; bg: string; text: string; border: string }[] = [
  ...
  { id: 'critique', label: 'UI/UX Critique', icon: PaletteIcon, bg: 'bg-purple-50 dark:bg-purple-950/40', text: 'text-purple-700 dark:text-purple-300', border: 'border-purple-200 dark:border-purple-800/60' },
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <div className="w-screen max-w-lg bg-white dark:bg-gray-900 shadow-2xl border-l border-gray-100 dark:border-gray-800 flex flex-col h-full">
              ...
              <div className="p-5 border-b border-gray-100 dark:border-gray-800 bg-gradient-to-br from-blue-50/50 via-white to-purple-50/30 dark:from-gray-900 dark:to-gray-850">
                  ...
                  <div className="flex items-center gap-2">
                    {isSuperAdmin && (
                      <Link
                        to="/feedback"
                        onClick={() => setIsOpen(false)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/50 border border-blue-200/60 dark:border-blue-800/40 text-[11px] font-bold text-blue-700 dark:text-blue-300 hover:bg-blue-100 transition cursor-pointer"
                    ...
                    <button
                      onClick={() => setIsOpen(false)}
                      className="rounded-xl p-1.5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer"
                    >
                ...
                {/* Current Page Context Tag */}
                <div className="mt-3 flex items-center gap-2 px-3 py-1.5 rounded-xl bg-blue-100/60 dark:bg-blue-950/40 border border-blue-200/60 dark:border-blue-900/40 text-xs font-semibold text-blue-800 dark:text-blue-300">
                ...
                {isSuperAdmin && (
                  <div className="mt-4 flex rounded-xl bg-gray-100 dark:bg-gray-800 p-1">
                    ...
                    >
                      ...
                      <span className="px-1.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/50 text-[10px] font-extrabold text-blue-700 dark:text-blue-300">
                        {total}
                      </span>
                    </button>
                  </div>
                ...
                {isSubmittedSuccess && !isSuperAdmin ? (
                  <div className="py-10 px-4 text-center rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200/60 dark:border-emerald-900/40 space-y-3">
                    ...
                    <button
                      ...
                      className="mt-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-xs hover:bg-blue-700 transition cursor-pointer"
                    >
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                          ...
                          return (
                            <button
                              ...
                              className={`p-2.5 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between ${
                                category === cat.id
                                  ? `${cat.bg} ${cat.border} ring-2 ring-blue-500/40`
                                  : 'border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800/40 hover:bg-gray-50'
                              }`}
                            >
                              <span className="mb-1">
                                <IconComp className={`w-4 h-4 ${category === cat.id ? cat.text : 'text-gray-500'}`} />
                              </span>
                              <span className={`text-xs font-bold ${category === cat.id ? cat.text : 'text-gray-700 dark:text-gray-300'}`}>
                                {cat.label}
                              </span>
                            </button>
                          );
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className="text-xs font-semibold text-amber-500">
                          {rating ? `${rating} / 5 Stars` : 'Optional'}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <button
                            ...
                            className={`p-1 transition-transform hover:scale-125 focus:outline-none cursor-pointer ${
                              rating && star <= rating ? 'text-amber-400' : 'text-gray-300 dark:text-gray-700'
                            }`}
                          >
                            <StarIcon filled={!!(rating && star <= rating)} className="w-6 h-6" />
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3.5 py-2.5 text-xs text-gray-900 dark:text-white placeholder-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                      />
                    </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        {(['low', 'medium', 'high', 'urgent'] as FeedbackPriority[]).map((p) => (
                          <button
                            ...
                            className={`px-2.5 py-1 text-[11px] font-bold rounded-lg uppercase tracking-wider transition-colors capitalize cursor-pointer ${
                              priority === p
                                ? p === 'urgent'
                    ...
                    <button
                      ...
                      className="w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 py-3 text-xs font-bold text-white shadow-md shadow-blue-500/20 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-50 transition-all cursor-pointer flex items-center justify-center gap-2"
                    >
                      {submitMutation.isPending ? (
                        ...
                        <>
                          <span>Submit Critique &amp; Suggestion</span>
                        </>
                      )}
                    </button>
                  </form>
